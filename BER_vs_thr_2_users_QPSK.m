...
for ii = 1:iter
    ...
    for thr_iter = 1:length(thrld)
        ...
        X_DD_u1(17:25,1:4) = sqrt((1/2)*SNR)*((2*randi([0,1],9,4)-1)+1i*(2*randi([0,1],9,4)-1));
        X_DD_u1(1:32,5:16) = sqrt((1/2)*SNR)*((2*randi([0,1],32,12)-1)+1i*(2*randi([0,1],32,12)-1));
        X_DD_u2(1:32,17:29) = sqrt((1/2)*SNR)*((2*randi([0,1],32,13)-1)+1i*(2*randi([0,1],32,13)-1));
        X_DD_u2(17:25,29:32) = sqrt((1/2)*SNR)*((2*randi([0,1],9,4)-1)+1i*(2*randi([0,1],9,4)-1));
